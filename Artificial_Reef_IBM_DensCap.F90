...
MODULE allvar
  ...
  REAL*8 :: rigdenscap  ! density caps for each reef type. 30% greater than values obtained from chapters 1, 2, and 3 for Red Snapper
  REAL*8 :: pyrdenscap
  REAL*8 :: natdenscap
...
!           1052      FORMAT(1x,e12.3,1x,6(i5,1x),i10,1x,3(i4,1x),1x,i3,1x,10(f12.4,1x))
           ...
           IF(iyear.eq.10.and.iday.eq.180.and.ihour.eq.9.and.(xspecies(ifish).eq.6.or.xspecies(ifish).le.3)) THEN
...
! determine nearest rig cell for each fish for each hour
  IF (iyear.le.5) then ! higher density caps for the first 10 years because so many individuals on the model grid
  rigdenscap = 0.0349*120  ! density caps for each reef type. 30% greater than values obtained from chapters 1, 2, and 3 for Red Snapper
  ELSE IF (iyear.gt.5.and.iyear.le.10) then ! once the model is stabilized. reduce density caps to represent realistic values
  rigdenscap = 0.0349*120  ! density caps for each reef type. 30% greater than values obtained from chapters 1, 2, and 3 for Red Snapper
  ELSE  ! once the model is stabilized. reduce density caps to represent realistic values
  rigdenscap = 0.0349*120  ! density caps for each reef type. 30% greater than values obtained from chapters 1, 2, and 3 for Red Snapper
  END IF
  xsp = xspecies(ifish)
...
!  zdensrig = 0
  olddistrig=nrow*ncol*cellsize  ! largest distance that is possible to start with
       do k=1,nrigs  ! search over the list of rig cells
           ...
           IF(iday.eq.2.and.ifish.eq.2) then
               PRINT *, ihour, zdensrig, rigdenscap
           END IF
           ...
           IF(xsp.eq.1) then
               IF(distrig.lt.olddistrig.and.zdensrig.le.rigdenscap)then ! see if shortest so far in search through the list of rig cells. 
...
!  zdenspyr = 0
  olddistpyr=nrow*ncol*cellsize  ! largest distance that is possible to start with
       do k=1,npyr  ! search over the list of rig cells
           ...
           IF(xsp.eq.1) then 
               IF(distpyr.lt.olddistpyr.and.zdenspyr.le.pyrdenscap)then ! see if shortest so far in search through the list of rig cells. 30% greater than the observed densities b/c we sample during the day - KM 5/25
...
!  zdensnat = 0
  olddistnat=nrow*ncol*cellsize  ! largest distance that is possible to start with
       do k=1,numnatcells  ! search over the list of rig cells
           ...
           IF(xsp.eq.1)then
           IF(distnat.lt.olddistnat.and.zdensnat.le.natdenscap)then ! see if shortest so far in search through the list of rig cells. 30% greater than the observed densities b/c we sample during the day - KM 5/25
